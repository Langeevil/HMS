...
<?php render_admin_sidebar($basePath, 'leitos'); ?>
        <main class="col-lg-9 col-xl-10 content">
            <div class="page-toolbar"><div><h1 class="h2 fw-bold mb-1">Novo leito</h1><p class="text-muted mb-0">Formulário para cadastro e edição de leitos hospitalares.</p></div><div class="toolbar-actions"><?php render_user_menu($usuarioLogado['nome'] ?? 'Usuário', url($basePath, 'pages/logout.php')); ?></div></div>
...
<?php endif; ?>
                ...
                <div class="row g-3">
                    <div class="col-md-4"><label for="numero" class="form-label">Número do leito</label><input type="number" name="numero" value="<?= h($leito['numero'] ?? '') ?>" class="form-control" id="numero" required></div>
                    <div class="col-md-4"><label for="codquarto" class="form-label">Quarto</label><select name="codquarto" class="form-select" id="codquarto" required><option value="">Selecione...</option>
...
<?php endforeach; ?>
                    ...
                    <div class="col-md-4"><label for="status" class="form-label">Status</label><select name="status" class="form-select" id="status" required>
